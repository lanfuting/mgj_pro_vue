...
<template>
  <div class="dv_scrtop">
    ...
    <div class="dv_scrtop_lists" @mouseover="scroll_stop()" @mouseout="scroll_start()">
      <div v-for="(s,i) in scroll_pts" :style="s" class="dv_scrtop_left" v-if="i==scroll_index"></div>
      <div class="dv_scrtop_btns">
         <button :class="{'dv_scrtop_btn_color':i==scroll_index}" @mouseover="change_pt(i)" class="dv_scrtop_btn" v-for="(l,i) in scroll_pts.length"></button>
      </div>
    </div>
    <!-- 导航菜单 -->
...
<script>
  export default{
    data:function () {
      return{
        scroll_pts:[
          "background-image: url('http://127.0.0.1:8082/backstage/imgs/rolladv/gg1.png')",
          "background-image: url('http://127.0.0.1:8082/backstage/imgs/rolladv/gg2.png')",
          "background-image: url('http://127.0.0.1:8082/backstage/imgs/rolladv/gg3.png')"
        ],
        scroll_index:0,
        scroll_pt:"",
        btn_color:false,
    ...
    methods:{
      ...
      scrollpt(){
        ...
        po.scroll_pt = window.setInterval(function(){
          po.scroll_index++;
           if(po.scroll_index==po.scroll_pts.length){
             po.scroll_index=0;
           }
        },2500);
      }
